...
static int lookup_named_color(const char *name) {
  ...
  if (strcmp(name, "white") == 0)
    return 7;
  if (strcmp(name, "brblack") == 0)
    return 8;
  if (strcmp(name, "brred") == 0)
    return 9;
  if (strcmp(name, "brgreen") == 0)
    return 10;
  if (strcmp(name, "bryellow") == 0)
    return 11;
  if (strcmp(name, "brblue") == 0)
    return 12;
  if (strcmp(name, "brmagenta") == 0)
    return 13;
  if (strcmp(name, "brcyan") == 0)
    return 14;
  if (strcmp(name, "brwhite") == 0)
    return 15;
...
static int parse_color_value(const char *value, int *attr) {
  char buf[256];
  size_t vlen = min(strlen(value), sizeof(buf) - 1);
...
static void scan_theme_dir(const char *dir) {
  ...
  while ((entry = readdir(d)) != nullptr) {
    ...
    snprintf(theme_names[theme_count], sizeof(theme_names[0]), "%s",
             display_name);
...
void apply_startup_theme(void) {
  if (!theme_name[0] || strcmp(theme_name, "default") == 0)
    return;
  scan_themes();
